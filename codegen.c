...
bool code_generator_start()
{
    ...
    if (!generate_file_header())
        return false;

    if (!define_built_in_functions())
        return false;
...
bool generate_function_retval(Data_type type)
{
    ...
    if (!generate_default_var_value(type))
        return false;
    ADD_CODE("\n");
...
static bool generate_term_value(Token token)
{
    ...
    if (!dynamic_string_init(&tmp_string))
        return false;
...
static bool generate_label(char *function_id, int label_index, int label_deep)
{
    ...
    ADD_CODE(function_id);
    ADD_CODE("%");
    ADD_CODE_INT(label_deep);
    ADD_CODE("%");
    ADD_CODE_INT(label_index);
    ADD_CODE("\n");
...
bool generate_if_start(char *function_id, int label_index, int label_deep)
{
    ...
    ADD_CODE("%");
    ADD_CODE_INT(label_index);
    ADD_CODE(" GF@%exp_result bool@false\n");
...
bool generate_if_else_part(char *function_id, int label_index, int label_deep)
{
    ...
    ADD_CODE_INT(label_deep);
    ADD_CODE("%");
    ADD_CODE_INT(label_index + 1);
    ADD_CODE("\n");

    ADD_INST("# Else");

    if (!generate_label(function_id, label_index, label_deep))
        return false;
